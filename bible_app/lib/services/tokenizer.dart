...
class BertTokenizer {
  ...
  // Replicates BERT's BasicTokenizer: lowercase → NFD-strip-accents.
  // Without this, Portuguese text like "poço" stays as-is and maps to [UNK]
  // instead of the expected "poco", producing embeddings in a different space
  // from those generated by @xenova/transformers on the server side.
  ...
  ({List<int> inputIds, List<int> attentionMask, List<int> tokenTypeIds})
    ...
    final normalized = _normalizeText(text.toLowerCase());
    final tokens = <String>[_clsToken];
    for (final word in _basicTokenize(normalized)) {
